...
Real ScalarField::my_phi_function(const RealVect &loc) const
{
    ...
    double dPhi_L = *(m_matter_params.dPhi + indxL);
    double dPhi_H = *(m_matter_params.dPhi + indxH);
    ...
    double phi =
    m_matter_params.dPhi[indxL] +
        (rr / spacing - indxL) * (dPhi_H - dPhi_L);

    int ell_SH = 1;
    int m_SH = 1;
    int s_SH = 0;

    SphericalHarmonics::Y_lm_t<double> spi_Y_2Plusl_m = SphericalHarmonics::spin_Y_lm(loc[0], loc[1], loc[2], s_SH, 2.0 + ell_SH, m_SH);
    SphericalHarmonics::Y_lm_t<double> spi_Y_l_m = SphericalHarmonics::spin_Y_lm(loc[0], loc[1], loc[2], s_SH, ell_SH, m_SH);
    ...
    double phi_tot = 0.01 * phi * phi_ang_real;
...
Real ScalarField::my_Pi_function(const RealVect &loc) const
{
    ...
    double phi_tot = - 0.01 * phi * phi_ang_imag;

    return m_matter_params.scalar_mass * phi_tot;
}
